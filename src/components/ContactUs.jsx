...
viewport={{once: true}}
   onSubmit={onSubmit} className='grid sm:grid-cols-2 gap-3 sm:gap-5 max-w-2xl w-full'>
    ...
    <div className='sm:col-span-2'>       
         ...
         <textarea name='message' rows={8} placeholder='Enter your message' className='w-full bg-transparent p-3 text-sm outline-none rounded-lg border border-gray-300 dark:border-gray-600' required/>
    </div>
    <button type='submit' className='bg-primary hover:scale-105 duration-500 transition-all w-max flex gap-2 text-white text-sm py-3 px-10 rounded-full cursor-pointer items-center'>Submit <img src={assets.arrow_icon} alt="" /></button>
   </motion.form>
    </motion.div>
  )
}
